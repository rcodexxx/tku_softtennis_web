...
<style scoped>
  .quick-register-page-naive {
    ...
    min-height: calc(
      100vh - var(--header-height, 64px) - var(--footer-height, 67px)
    ); /* 確保CSS變數已定義或替換為實際值 */
    padding: 20px;
  ...
  /* 卡片副標題/說明文字 */
  .register-card .card-subtitle-naive.n-p {
    /* 針對 n-p 元素 */
  ...
  /* Alert 間距 */
  .form-alert.n-alert {
    /* 使用 form-alert class 統一 alert 間距 */
    margin-bottom: var(--n-form-item-margin-bottom, 24px) !important;
  }

  /* 註冊按鈕 */
  .main-submit-button.n-button {
    /* .register-button 已改名為 .main-submit-button */
    font-weight: 500;
    /* size="large" 已設定高度和大部分樣式 */
  }

  /* 分隔線 */
  .form-action-divider.n-divider:not(.n-divider--vertical) {
    /* 使用 form-action-divider class */
    margin-top: 1.8rem;
