...
class MatchRecord(db.Model):
    ...
    def get_all_games_scores_with_serve(self) -> list[dict]:
        ...
        for game_num in range(1, 10):
            ...
            if a_score > 0 or b_score > 0:
                ...
                # 🔥 新增發球資訊
                serve_side = self._get_serve_side_for_game(game_num)
                if serve_side:
                    game_detail.update(
                        {
    ...
    def _get_serve_side_for_game(self, game_number: int) -> str:
        """
        獲取指定局數的發球方
        """
        ...
        # 獲取第一局發球方
        first_serve = self.first_serve_side.value

        if game_number % 2 == 1:
            return first_serve
        else:
            # 偶數局切換發球方
            return "side_b" if first_serve == "side_a" else "side_a"
    ...
    def get_serving_players_for_game(self, game_number: int) -> list:
        ...
        Returns:
            list: 發球方球員ID列表
        """
        serve_side = self._get_serve_side_for_game(game_number)
        if not serve_side:
            return []
